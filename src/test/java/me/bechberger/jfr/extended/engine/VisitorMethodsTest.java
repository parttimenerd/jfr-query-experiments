...
/**
 * Comprehensive tests for the newly implemented visitor methods in QueryEvaluator.
...
class VisitorMethodsTest {
    ...
    void testShowEventsCommand() {
        ...
        assertEquals("Event Type", result.getTable().getColumnName(0), "Column should be named 'Event Type'");
        
        // Verify some common JFR event types are present
        var eventTypes = result.getTable().getColumn(0);
        boolean hasGCEvent = eventTypes.stream()
            .anyMatch(cell -> cell.toString().contains("GarbageCollection") || 
                             cell.toString().contains("jdk.GarbageCollection"));
        assertTrue(hasGCEvent, "Should include garbage collection events");
    }
    ...
    void testShowFieldsWithValidEventType() {
        ...
        assertTrue(eventsResult.getTable().getRowCount() > 0);
        ...
        assertTrue(fieldsResult.getTable().getColumnCount() >= 2, "Should have at least 2 columns");
        assertEquals("Field Name", fieldsResult.getTable().getColumnName(0), "First column should be 'Field Name'");
        assertEquals("Type", fieldsResult.getTable().getColumnName(1), "Second column should be 'Type'");
    }
    ...
    void testShowFieldsWithInvalidEventType() {
        ...
        if (result.isSuccess()) {
            assertEquals(0, result.getTable().getRowCount(), 
                "Non-existent event type should return empty results");
        } else {
            assertNotNull(result.getError(), "Should have error information");
            assertTrue(result.getError().getMessage().contains("NonExistentEventType") ||
                      result.getError().getMessage().contains("not found") ||
                      result.getError().getMessage().contains("unknown"),
                "Error message should mention the invalid event type");
        }
    }
    ...
    void testGeneralHelpCommand() {
        ...
        assertEquals(1, result.getTable().getColumnCount(), "Should have exactly one column");
        assertEquals("Help Content", result.getTable().getColumnName(0), "Column should be named 'Help Content'");
    ...
    void testHelpGrammarCommand() {
        ...
        assertEquals(1, result.getTable().getRowCount(), "Should have exactly one row");
        assertEquals(1, result.getTable().getColumnCount(), "Should have exactly one column");
        assertEquals("Grammar Documentation", result.getTable().getColumnName(0), "Column should be named 'Grammar Documentation'");
