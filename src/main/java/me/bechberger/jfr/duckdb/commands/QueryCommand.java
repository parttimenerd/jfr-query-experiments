...
public class QueryCommand implements Runnable {
    ...
    @CommandLine.Parameters(index = "1", description = "The SQL query to execute, if query is just a single word, it is interpreted as a view name and empty query lists available tables and views.", defaultValue = "")
    ...
    private void handleView(DuckDBConnection conn, PrintStream out, String query) throws SQLException {
        ...
        if (availableViews.contains(query)) {
            ...
            List<String> suggestions = TextUtil.findClosestMatches(query, getAvailableTablesAndViews(conn), 3, 5);
    ...
    private List<String> getAvailableViews(DuckDBConnection conn) throws SQLException {
        try (var stmt = conn.createStatement();
             var rs = stmt.executeQuery("SHOW VIEWS")) {
