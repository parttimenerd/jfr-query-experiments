...
public class QueryErrorMessageGenerator {
    ...
    public String generateFunctionSuggestion(String invalidName, List<ExpressionNode> arguments, QueryContext context) {
        ...
        candidateFunctions = candidateFunctions.stream()
            ...
            .sorted((f1, f2) -> {
                int similarity1 = calculateLevenshteinDistance(invalidName.toUpperCase(), f1.name());
                int similarity2 = calculateLevenshteinDistance(invalidName.toUpperCase(), f2.name());
    ...
    private String findSimilarFunctionName(String invalidName) {
        ...
        for (FunctionDefinition func : functionRegistry.getAllFunctions()) {
            int distance = calculateLevenshteinDistance(upperInvalidName, func.name());
